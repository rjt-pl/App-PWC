...
=head1 SYNOPSIS
    ...
    my $conf = App::PWC::Config->new( path => "/path/to/pwc.yaml" );
    $conf->read;
...
Creates a new configuration object. Optionally takes a path argument:
...
=head1 LOGGING CONFIG

This section includes all logging configuration as defined in the C<log:>
...
Returns the B<text> representation of the I<minimum> log level for the log
file, or the console log.


=head2 log_strftime

=head2 console_strftime

Returns the L<POSIX> C<strftime()> format string for the log file, or console
log.


=head2 gzip_logs

Returns TRUE if C<gzip_logs> is C<True> in the configuration. This will cause
rotated log files to be gzipped using L<Compress::Zlib>.


=head2 num_logs

Returns the number of old log files to keep, in addition to the most recent
rebuild log. If this number is set to 3, for example, after several runs, the
...
The path and filename of the base rebuild log. When the log files are rotated,
a number will be appended, as shown in the C<num_logs> example, above.


=head2 config_file

The path and filename of the configuration file itself.
